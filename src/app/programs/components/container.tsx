...
export const ProgramContainer: React.FC<ProgramContainerProps> = ({ program }) => {
    ...
    return (
        <div className="hero-container cursor-pointer text-[#0F973D]" onClick={handleClick}>
            <Container className="min-h-96 mt-10 sm:mt-10 md:mt-10 border-2 border-spacing-3 border-[#0F973D] rounded-2xl bg-[#B5E3C4] m-4 p-4 sm:p-6">
                <FadeIn className="max-w-3xl">
                    <div className="mt-4">
                        <Badge className="bg-[#0F973D] hover:bg-[#163922] text-white rounded-3xl px-4 sm:px-8 py-1 sm:py-2">{program.badge}</Badge>
                    </div>
                    ...
                    <div className="flex justify-center bg-white gap-2 sm:gap-4 border-2 mt-2 rounded-sm border-black p-2">
                        <div className="flex items-center">
                            <p className="text-black font-bold sm:text-balance">{program.startDate}</p>
                            <ThickArrowRightIcon className="h-4 font-bold mx-2 sm:mx-4" />
                            <p className="text-black font-bold sm:text-balance">{program.endDate}</p>
                        </div>
                    </div>
                    <hr className="border-2 my-4 sm:my-6 border-x border-black" />
                    <div className="mt-1 sm:text-center">
                        <h1 className='text-black'>{program.isCurrent ? `Join the ${program.name} train!` : 'Application Is Closed'}</h1>
                        <div>
                            <Button className="bg-[#0F973D] hover:bg-[#163922] mt-4" href={program.link} invert={invert}>
